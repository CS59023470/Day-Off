...
//API ค้นหารายการ ขอลางาน ทั้งหมดที่ได้รับการอนุมัติ สำหรับ หน้า report แค่ Sick Leave
async function queryAllLeaveForReportJustSick(req, res) {
    const doc = new GoogleSpreadsheet(sheet_api.sheetId);
    await promisify(doc.useServiceAccountAuth)(creds);
    const info = await promisify(doc.getInfo)();
    const sheet = info.worksheets[sheet_api.indexSheetLeave];
...
//API ค้นหารายการ ขอลางาน ทั้งหมดที่ได้รับการอนุมัติ สำหรับ หน้า report แค่ Personal Leave
async function queryAllLeaveForReportJustPersonal(req, res) {
    const doc = new GoogleSpreadsheet(sheet_api.sheetId);
    await promisify(doc.useServiceAccountAuth)(creds);
    const info = await promisify(doc.getInfo)();
    const sheet = info.worksheets[sheet_api.indexSheetLeave];
    ...
    const rowDatas = await promisify(sheet.getRows)({
        query: 'status != ""&type = "Personal Leave"'
    });
...
//API ค้นหารายการ ขอลางาน ทั้งหมดที่ได้รับการอนุมัติ สำหรับ หน้า report แค่ Vacation Leave
async function queryAllLeaveForReportJustVacation(req, res) {
    const doc = new GoogleSpreadsheet(sheet_api.sheetId);
    await promisify(doc.useServiceAccountAuth)(creds);
    const info = await promisify(doc.getInfo)();
    const sheet = info.worksheets[sheet_api.indexSheetLeave];
    ...
    const rowDatas = await promisify(sheet.getRows)({
        query: 'status != ""&type = "Vacation Leave"'
    });
